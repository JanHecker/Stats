...
public class SetStatsCommmand implements CommandExecutor {

    private final String prefix = "§6FeelPvP §8♦ §7 ";

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!sender.hasPermission("FeelPvP.SetStats")) {
            sender.sendMessage(prefix + "§cKeine Rechte!");
            return true;
        }
        // setstats <Name> <--> <i>
        // bsp /setstats LuckyLemonDE onlinetime 0
        String usage = prefix + "§cBenutze /setstats <Name> <deaths/onlinetime/kills/coins> <Wert>";
        if (args.length != 3) {
